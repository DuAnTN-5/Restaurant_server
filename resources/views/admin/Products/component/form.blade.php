<form method="POST" action="{{ isset($product) ? route('products.update', $product->id) : route('products.store') }}" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <div class="form-group row">
                <div class="col-lg-4">
                    ...
                    <input type="text" id="price" name="price" class="form-control" placeholder="VND" value="{{ old('price', $product->price ?? '') }}" oninput="formatCurrency(this)" required>
                </div>

                <div class="col-lg-4">
                    <label for="discount_price">Giá giảm:</label>
                    <input type="text" id="discount_price" name="discount_price" class="form-control" placeholder="VND" value="{{ old('discount_price', $product->discount_price ?? '') }}" oninput="formatCurrency(this)">
                </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="ingredients" name="ingredients" class="form-control" placeholder="Nhập nguyên liệu... (nhấn Enter để thêm)" value="{{ old('ingredients', $product->ingredients ?? '') }}">
            </div>
        </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="form-group">
                ...
                <select name="category_id" class="form-control" required>
                    ...
                    @foreach ($productCategories as $productCategory)
                        <option value="{{ $productCategory->id }}" {{ old('category_id', $product->category_id ?? '') == $productCategory->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="text" id="tags" name="tags" class="form-control" placeholder="Nhập tags... (nhấn Enter để thêm)" value="{{ old('tags', $product->tags ?? '') }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control" required>
                    <option value="active" {{ old('status', $product->status ?? 'active') == 'active' ? 'selected' : '' }}>Kích hoạt</option>
                    <option value="inactive" {{ old('status', $product->status ?? '') == 'inactive' ? 'selected' : '' }}>Vô hiệu</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="form-group text-right">
        <button type="submit" class="btn btn-primary">{{ isset($product) ? 'Cập Nhật Sản Phẩm' : 'Lưu Sản Phẩm' }}</button>
